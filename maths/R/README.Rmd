...
- [slides as dynamic html](https://github.com/ULHPC/tutorials/raw/devel/maths/R/Intro_PS.html)
...
see [the tutorial](https://cdn.rawgit.com/ULHPC/tutorials/raw/devel/maths/R/practical_datasaurus.html)
...
We could do a for loop to aggregate the data per cuts and manually compute the average price, but in R loops are generally a bad idea (if you avoid growing vectors there are fine). It is better to concentrate on actions and not on programming, so the looping can be efficiently done internally to available functions.
...
on the `iris` frontend, in the folder where both the **tsne.R** and **launcher.sh** files are located, simply run:
...
When you _passive_ job has started, you can connect to the assigned node using the `sjoin` command by Valentin Plugaru, use <TAB> to autocomplete the correct job and node ids.
...
see below the benefit of using more cores on the elapsed time for computationally intensive tasks such as t-SNE
