...
```{targets gapminder}
list(
  tar_target(gap, gapminder, packages = "gapminder"),
  tarchetypes::tar_group_by(gap_by, gap, continent, country),
  #tar_target(group, gap_by, pattern = map(gap_by)),
  tar_target(model, gap_lm(gap_by), pattern = map(gap_by),
             iteration = "group"),
  tar_target(r2, extract_r2(model), pattern = map(model),
...
tarchetypes::tar_render(report, "report_gap.Rmd")
```
...
`reporter = "summary"` to avoid to see :
