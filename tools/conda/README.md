...
# Environment management of projects with Conda
...
The R system can install packages in 3 different modes:

- system wide installations for packages accessible to all users,
- user installation for packages accessible for the current user only, and
- project installations for packages accessible only locally within a project directory.

In your local system you can install packages in any mode. In the HPC systems, you can only install packages in the user space, so you are limited to user and project installations. Nevertheless, the HPC installation of R includes a number of commonly used packages, such as `dbplyr` and `tidyverse`. You should check if the package you require is installed and that the installed version provides the functionality you need before installing any packages locally. Remember, local package installations consume space and inodes against personal and project quota.


Be default R installs packages system wide. In the HPC system, R detects that it does not have access to the system directories and suggests installing packages for the current user only. 
...
If you have problems with the space used by R, you can create project-wide installations of packages. You can change the installation paths with in a single session:
```R
project_path <- paste0( Sys.getenv("PROJECTHOME"), "project_name/", "R/x86_64-pc-linux-gnu-library/4.0" )
newpaths <- c( project_path, .libPaths() )
.libPaths( newpaths )
#strings.TrimRight(path, "/")
```
To permanently change the package search path, you need to modify your `R_LIBS` path, and this can be done in the `~/.Renviron` directory.
